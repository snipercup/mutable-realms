...
`GET /api/worlds/{world_id}/map` without a hierarchy scope preserves the legacy flat map. The scoped response returns the current location as the first scope node plus bounded direct children and promoted landmarks; the player-facing map renderer uses the scope node as the current player anchor and does not treat it as one of its own child locations. Scoped maps use the SVG as the child-location index rather than repeating those children in a separate list. Boundary links remain separately exposed and rendered as exits. Internal `location_links` remain authoritative for narration and movement validation but are not rendered as scoped-map edges; flat legacy maps retain their existing link rendering. Links crossing the visible boundary appear in `boundary_links` and do not create inferred adjacency. Map zoom and scope selection are read-only presentation actions.
...
`POST /api/worlds/{world_id}/locations/expand` exposes the same contract for deterministic integration checks. MCP `world_expand_location` is the narrator seam; it requires explicit world/revision/operation/proposal identifiers and resolves the actor from the trusted player binding when available. Exact operation replay returns `already_applied: true` without another revision; reusing a proposal ID for a different operation is rejected.
