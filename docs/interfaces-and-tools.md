...
- `location_metadata(world_id, location_id, kind, geography_role, direction, range_band, is_map_scope, is_default_scope)` stores descriptive kind plus explicit transition/route-orientation metadata. `geography_role` is `local`, `boundary`, or `route`; `direction` is cardinal/intercardinal; `range_band` is `short`, `mid`, or `long`. These fields orient presentation and do not grant movement.
