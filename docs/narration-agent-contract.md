...
- Supply only allowed `geography_role` (`local`, `boundary`, `route`) and `map_form` (`building`, `street`, `district`, `city`, `mine`, `forest`, `water`, `gate`, `landmark`) values; every non-null `parent_name` must match a location in the returned array.
