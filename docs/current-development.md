...
### Narrator-driven contextual starting locations — proposed

**Goal:** when **Begin your story** starts a world, the player should begin at the location implied by the opening scene rather than automatically occupying the named landmark. For Aerthalon, an opening scene that places the player “on the way to” or “in front of the doors” of the Adventurer's Guild should create or select **Street of Elaris** as the player's current location, with the Adventurer's Guild as a child location within that street. The resulting map should contain that minimal meaningful structure instead of only one flat starting location.

**Proposed scope:** extend the structured narrator-start contract and atomic world-start operation so the narrator can distinguish the player's physical starting position from a nearby landmark, create the bounded parent/child location structure when needed, and place the player at the contextual street/approach location. Preserve containment-versus-movement semantics: the guild being a child of the street does not by itself create movement adjacency. Keep creation bounded, revision-aware, idempotent, and compatible with existing flat worlds and already-created locations.

**Out of scope:** unrestricted opening-scene geography generation, automatic roads or routes, inferring travel from containment, rewriting existing locations silently, or changing the authoritative meaning of an already-started world. Existing worlds should require an explicit migration/repair path or remain unchanged rather than being regenerated by pressing **Begin your story** again.

**Verification:** pending implementation. The eventual checks should use a temporary Aerthalon-like world and confirm the structured start response, player placement at Street of Elaris, Adventurer's Guild containment, bounded map output, one revision increment, exact replay, and unchanged behavior for a simple single-location opening.
