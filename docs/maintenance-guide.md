...
25. **Scenario AI instructions are copied, not shared.** `ai_instructions` is an optional scenario element copied into `world_elements` during instancing. The prompt orders it before author note, plot essentials, and opening scene; editing either source afterward does not mutate the other.
26. **Road and gate scope decorations are presentation-only.** When a scoped location is semantically a route/road or gate, the frontend may draw deterministic SVG scenery (road surface/trees/rocks or a gatehouse/wall/road) even when there are no child locations. These layers are derived from `map_form`/scope metadata; a narrow name fallback supports legacy gate records. They never imply child locations, movement links, reachability, events, or world revisions.

## System inventory

The full command, API, MCP tool, and environment-variable reference lives in [interfaces-and-tools.md](interfaces-and-tools.md). At a glance: 24 additive migrations (`0001` initial through `0024` gate_map_form); the agent-facing atomic operations (`world_move_entity`, `world_travel_route`, `world_create_route`, `world_expand_location`, `world_treat_and_discharge_patient`, `world_record_social_interaction`, `world_record_location_memory`, `world_consolidate_location_memories`, `world_transfer_resource`, `world_update_location`); revision-aware administrative services for scenarios (including region framework authoring), world instancing, location hierarchy, cross-scale landmark promotion, route definitions, and bounded location expansion; and the `POST /api/worlds/{world_id}/turns` relay (deterministic with `decision_json`, narrated without it).
