...
11. **Scoped maps are bounded derived views.** The API may return the current scope node plus direct children, bounded sibling neighbors, explicitly promoted landmarks, exact and projected player positions, visible links, and separate boundary exits. Sibling neighbors share the current location's parent; parentless roots use the virtual world scope. The scoped frontend draws the current scope node as the player anchor, uses the SVG as the child/neighbor location index, does not repeat children in a second list, and does not draw internal `location_links` as edges. Neighbor visibility never proves movement. Boundary exits remain visible. It must use stable ordering and explicit overflow metadata rather than loading an unbounded hierarchy into the browser or narrator context. Flat worlds remain valid roots and retain the legacy map fallback.
