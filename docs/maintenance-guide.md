...
11. **Scoped maps are bounded derived views.** The API may return scope metadata plus direct children, explicitly promoted landmarks, exact and projected player positions, visible links, and separate boundary exits. The scoped frontend uses the SVG as the child-location index: it does not draw the scope anchor as a node inside itself, repeat children in a second list, or draw internal `location_links` as edges. Boundary exits remain visible. It must use stable ordering and explicit overflow metadata rather than loading an unbounded hierarchy into the browser or narrator context. Flat worlds remain valid roots and retain the legacy map fallback.
